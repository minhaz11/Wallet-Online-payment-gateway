...
class GeneralSettingController extends Controller
{
    ...
    {
            $request->validate([
                'template' => ['required','image',new FileTypeValidate(['jpg','jpeg','png'])]
            ]);
            $general = GeneralSetting::first();
             if ($request->hasFile('template')) {
                try {
                    $old = $general->qr_template ?: null;
                    $general->qr_template = uploadImage($request->template, 'assets/images/qr/', '2480x3508', $old);
                } catch (\Exception $exp) {
