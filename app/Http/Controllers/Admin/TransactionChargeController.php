...
class TransactionChargeController extends Controller
{
    ...
    {
        $request->validate([
            'percentage_charge' => 'required|numeric|between:0,100',
            'fixed_charge' => 'required|numeric|gte:0',
            'cap'   => 'numeric|gte:-1',
            'min_limit' => 'numeric|gte:0',
            'max_limit' => 'numeric|required|gt:min_limit',
            'commission' => 'numeric|gte:0',
        ...
        if($request->max_limit < $request->min_limit){
            $notify[]=['error','Maximum limit can not be less than minimum limit'];
        ...
        $charge->commission = $request->commission;
