...
class UserWithdrawController extends Controller
{
    ...
    public function withdrawMoney()
    {
        ...
        $userMethods = UserWithdrawMethod::where([['user_type','USER'],['user_id',auth()->id()],['status',1]])->whereHas('withdrawMethod')->get();
    ...
    {
        $userMethods = UserWithdrawMethod::where('user_type','USER')->where('user_id',auth()->id())->whereHas('withdrawMethod')->paginate(getPaginate());
    ...
    public function addWithdrawMethod()
    {
        ...
        return view($this->activeTemplate.'user.withdraw.add_method', compact('pageTitle','withdrawMethod','currencies'));
    }
    ...
    public function withdrawMethodStore(Request $request)
    {
        ...
        return redirect(route('user.withdraw.methods'))->withNotify($notify);
    ...
    public function withdrawStore(Request $request)
    {
        ...
        $wallet = Wallet::where('user_type','USER')->where('user_id',auth()->id())->where('currency_id',$currency->id)->first();
        if(!$wallet){
    ...
    public function withdrawMethodEdit($id)
    {
        ...
        $userMethod = UserWithdrawMethod::where('id',$id)->where('user_type','USER')->where('user_id',auth()->id())->whereHas('withdrawMethod')->first();
        if (!$userMethod) {
        ...
        $withdrawMethod = WithdrawMethod::whereJsonContains('user_guards',userGuard()['type'])->where('status',1)->get();
        $currencies = Currency::pluck('id','currency_code');
        return view($this->activeTemplate.'user.withdraw.withdraw_method_edit', compact('pageTitle','userMethod','withdrawMethod','currencies'));
    }

    public function withdrawMethodUpdate(Request $request)
    {
        $userMethod = UserWithdrawMethod::where('id',$request->id)->where('user_type','USER')->where('user_id',auth()->id())->first();
        if (!$userMethod) {
            $notify[]=['error','Withdraw method not found'];
            return back()->withNotify($notify);
        }
