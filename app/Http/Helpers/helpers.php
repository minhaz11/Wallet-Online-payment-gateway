...
{
    $user = userGuard()['user'];
    $qrCode = QRCode::where('user_id',$user->id)->where('user_type',userGuard()['type'])->first();
    if(!$qrCode){
        $qrCode = new QRcode();
        ...
        $qrCode->save();
        //return cryptoQR($qrCode->unique_code);
    } else {
        //return cryptoQR($user->qrCode->unique_code);
    }
    return $qrCode;
...
//moveable
function sendGeneralEmail($email, $subject, $message_data, $receiver_name = '')
{
    ...
    $message = shortCodeReplacer("{{fullname}}", $receiver_name, $general->email_template);
    $message = shortCodeReplacer("{{username}}",$receiver_name, $message);
    $message = shortCodeReplacer("{{message}}", $message_data, $message);
...
function module($key)
{
    return ModuleSetting::where('user_type',userGuard()['type'])->where('slug',$key)->first();
}
...
function userTrx($req){
  
    $search = $req->search;
    $search ? $pageTitle = "Search Result of #$search":$pageTitle = "Transaction History";;
    $type = $req->type;
    ...
    $histories = Transaction::where([['user_id',userGuard()['user']->id],['user_type',userGuard()['type']]])
    ...
    return view(activeTemplate().strtolower(userGuard()['type']).'.trx_history',compact('pageTitle','histories'));
}
...
function kycStyle(){
    ...
    else if(userGuard()['user']->kyc_status == 0) {
        $kyc['bgColor'] = '';
        $kyc['btnBg'] = '';
    ...
    else if(userGuard()['user']->kyc_status == 3) {
...
function totalProfit($date=null){
    $profits = ChargeLog::where([['operation_type','!=','add_money'],['operation_type','!=','withdraw_money']])->with(['currency','user','agent','merchant'])->when($date, function($q,$data){
...
function totalMoneyInOut($day = 7){
        $date = Carbon::today()->subDays($day);
        $moneyIn = Transaction::where([['user_type',userGuard()['type']],['user_id',userGuard()['user']->id],['trx_type','+'],['created_at', '>=', $date]])->with('currency')->get(['amount','currency_id']);

        $moneyOut = Transaction::where([['user_type',userGuard()['type']],['user_id',userGuard()['user']->id],['trx_type','-'],['created_at', '>=', $date]])->with('currency')->get(['amount','currency_id']);
