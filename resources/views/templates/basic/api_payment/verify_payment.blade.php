      ...
      <div class="row justify-content-center">
        <div class="col-lg-5 col-md-8">
          ...
          <div class="checkout-wrapper rounded-3  {{@$apiPayment->checkout_theme == 'dark' ? 'checkout-wrapper--dark':''}} shake-card">
            ...
            @php
              if(session('data')){
                $route = route('test.confirm.payment');
              }  else {
                $route = route('confirm.payment');
              }
            @endphp
            <div class="checkout-wrapper__header text-center">
              <img src="{{$apiPayment->site_logo}}" alt="image" class="form-logo mb-3">
              <h3 class="product-price mt-2">{{@$apiPayment->curr->currency_symbol}} {{showAmount($apiPayment->amount)}} {{@$apiPayment->curr->currency_code}}</h3>
              <h6 class="mb-5 title fw-normal mt-2">@lang('for') {{__($apiPayment->details)}}</h6>
            </div>
            <form class="mt-5" method="post" action="{{$route}}">
                @csrf
              <div class="form-group">
                <input type="text" name="code" class="form--control" required placeholder="@lang('Verification code')" autocomplete="off">
                @if (!session('data'))
                <a href="{{route('resend.code')}}" class="font-size--14px text--base mt-1">@lang('Resend code')</a>
                @endif
              </div>
            ...
            <div class="row mt-5">
              <div class="col-6">
                <p class="font-size--14px">@lang('Powered by') <a href="{{route('home')}}" class="text--secondary font-size--16px"><strong>{{$general->sitename}}</strong></a></p>
              </div>
              <div class="col-6">
