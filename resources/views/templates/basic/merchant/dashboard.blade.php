...
@section('content')
@if(merchant()->kyc_status == 0 ||  merchant()->kyc_status == 2 ||  merchant()->kyc_status == 3 && $userKyc->status == 1)
<div class="d-user-notification d-flex flex-wrap align-items-center mb-5 mt-4 {{$kyc['bgColor']}}">
    <div class="icon  {{ $kyc['iconBg']}}">
        <i class="las la-folder"></i> 
    </div>
    <div class="content">
        <p class="text-white"><b>@lang($kyc['msg'])</b> @if(merchant()->kyc_status == 3) <a class="btn btn--warning btn-sm reason" href="javascript:void(0)" data-reasons="{{merchant()->kyc_reject_reasons}}">@lang('See reasons')</a> @endif</p>
    </div>
...
<div class="d-flex justify-content-between">
    ...
    <a href="{{route('merchant.all.wallets')}}" class="font-size--14px text--base">@lang('More Wallets') <i class="las la-long-arrow-alt-right"></i></a>
   </div>
            ...
            <div class="col-sm-6">
                <div class="custom--card mb-4">
                    <div class="card-body">
                        ...
                        <h3 class="title fw-normal money-in">{{$general->cur_sym}}{{showAmount($totalMoneyInOut['totalMoneyIn'])}} {{$general->cur_text}}</h3>
                        <a href="{{url('merchant/transaction/history?type=money-in')}}" class="text--link text-muted font-size--14px">@lang('Total received')</a>
                        <div class="d-flex flex-wrap align-items-center justify-content-between mt-4">
                            <a href="{{url('merchant/transaction/history?type=money-in')}}" class="font-size--14px fw-bold">@lang('View Transactions')</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 mt-sm-0 mt-3">
                <div class="custom--card">
                    <div class="card-body">
                        ...
                        <h3 class="title fw-normal money-out">{{$general->cur_sym}}{{showAmount($totalMoneyInOut['totalMoneyOut'])}} {{$general->cur_text}}</h3>
                        <a href="{{url('merchant/transaction/history?type=money-out')}}" class="text--link text-muted font-size--14px">@lang('Total spent')</a>
                        <div class="d-flex flex-wrap align-items-center justify-content-between mt-4">
                            <a href="{{url('merchant/transaction/history?type=money-out')}}" class="font-size--14px fw-bold">@lang('View Transactions')</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="custom--card">
                    <div class="card-body">
                        ...
                        <h3 class="title fw-normal money-out">{{$general->cur_sym}}{{showAmount($totalWithdraw)}} {{$general->cur_text}}<sup>*</sup></h3>
                ...
                <div class="accordion-item transaction-item {{$history->trx_type == '-' ? 'sent-item':'rcv-item'}}">
                    ...
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#c-{{$loop->iteration}}" aria-expanded="false" aria-controls="c-1">
                        ...
                        <div class="col-lg-3 col-sm-3 col-6 order-sm-3 order-2 text-end amount-wrapper">
                            <p><b>{{showAmount($history->amount+$history->charge)}} {{$history->currency->currency_code}}</b></p>
...
@push('script')
<script src="{{asset($activeTemplateTrue.'merchant/js/apexcharts.min.js')}}"></script>
     <script>
            'use strict';
            (function ($) {
