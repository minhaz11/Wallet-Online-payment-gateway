          ...
          <div class="form-group">
            ...
            <select class="select" name="wallet_id" id="wallet"  required>
                    <option value="{{$wallet->id}}"
                     ...
                     data-type="{{$wallet->currency->currency_type}}" {{$invoice ? $invoice->currency_id == $wallet->currency->id ? 'selected':'':''}} data-gateways="{{$wallet->gateways()}}" >@lang($wallet->currency->currency_code)
                    </option>
             ...
             <label>@lang('Select Gateway')</label>
              <select class="select gateway" name="method_code" required>
                  ...
                  @foreach ($gateways as $item)
                    <option data-max="{{$item->max_amount}}" data-min="{{$item->min_amount}}" data-fixcharge = "{{$item->fixed_charge}}" data-percent="{{$item->percent_charge}}"  value="{{$item->method_code}}">{{$item->name}}</option>
                  @endforeach
              </select>
        ...
        <div class="add-moeny-card-middle">
          <ul class="add-money-details-list">
            <li>
              ...
              <div class="value"><span class="sym">{{$invoice->currency->currency_symbol}}</span> <span class="show-amount">{{showAmount($invoice->total_amount,2)}}</span></div>
            </li>
            <li>
          ...
          <div class="add-money-details-bottom">
            ...
            <div class="value"><span class="sym">{{$invoice->currency->currency_symbol}}</span> <span class="payable">{{showAmount($invoice->total_amount)}}</span> </div>
          </div>
        </div>
