            ...
            <div class="row gy-3 gx-3 p-3">
                ...
                <div class="col-xl-2 col-lg-3 col-md-4">
                    <div class="custom-select-box-two">
                        ...
                        <select onChange="window.location.href=this.value">
                            <option value="{{queryBuild('operation','')}}" {{request('operation') == '' ? 'selected':''}}>@lang('All Operation')</option>
                            <option value="{{queryBuild('operation','add_money')}}" {{request('operation') == 'add_money' ? 'selected':''}}>@lang('Add Money')</option>
                            <option value="{{queryBuild('operation','money_out')}}" {{request('operation') == 'money_out' ? 'selected':''}}>@lang('Money Out')</option>
                            <option value="{{queryBuild('operation','transfer_money')}}" {{request('operation') == 'transfer_money' ? 'selected':''}}>@lang('Transfer Money')</option>
                            <option value="{{queryBuild('operation','request_money')}}" {{request('operation') == 'request_money' ? 'selected':''}}>@lang('Request Money')</option>
                            <option value="{{queryBuild('operation','make_payment')}}" {{request('operation') == 'make_payment' ? 'selected':''}}>@lang('Make Payment')</option>
                            <option value="{{queryBuild('operation','exchange_money')}}" {{request('operation') == 'exchange_money' ? 'selected':''}}>@lang('Exchange Money')</option>
                            <option value="{{queryBuild('operation','create_voucher')}}" {{request('operation') == 'create_voucher' ? 'selected':''}}>@lang('Create Voucher')</option>
                            <option value="{{queryBuild('operation','withdraw_money')}}" {{request('operation') == 'withdraw_money' ? 'selected':''}}>@lang('Withdraw Money')</option>
                            <option value="{{queryBuild('operation','add_balance')}}" {{request('operation') == 'add_balance' ? 'selected':''}}>@lang('Add By System')</option>
                            <option value="{{queryBuild('operation','sub_balance')}}" {{request('operation') == 'sub_balance' ? 'selected':''}}>@lang('Subtract By System')</option>
                        </select>
                    </div><!-- custom-select-box-two end -->
                </div>
                <div class="col-xl-2 col-lg-3 col-md-4">
                    ...
                    <div class="custom-select-box-two">
                        ...
                        <select onChange="window.location.href=this.value">
                            ...
                            @foreach (userGuard()['user']->wallets as $item)
                              <option value="{{queryBuild('currency',strtolower($item->currency->currency_code))}}" {{request('currency') == strtolower($item->currency->currency_code) ? 'selected':''}}>{{$item->currency->currency_code}}</option>
                            @endforeach
                        </select>
                    </div><!-- custom-select-box-two end -->
                </div>
